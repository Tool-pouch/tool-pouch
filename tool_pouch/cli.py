"""CLI entry point.
    ...
    Test path
        tool-pouch init                                       # autodetect setup
        tool-pouch scan [path] [--quick] [--judge ...]        # decorator path
        tool-pouch run [<module>] [--format human|fix-prompt] # custom orchestration

    Production / replay
        tool-pouch traces [--agent ...] [--since 1h] [--failed] [--request-id RID]
        tool-pouch trace <trace_id|--request-id RID>
        tool-pouch replay <trace_id> [--frozen | --frozen-tools | --chaos]
                     [--repeat N] [--strict | --loose-tools | --match-closest]
        tool-pouch sync                                       # cloud upgrade pitch

    Inspection
        tool-pouch show <run_id> [--filter type]
        tool-pouch runs [--failed]
        tool-pouch fix-prompt [<run_id>]
        tool-pouch config                                     # set LLM judge provider
"""
import argparse
import asyncio
...
def cmd_run(args):
    ...
    if not agent_file:
        ...
        print()
        print("`tool-pouch run` is for custom orchestration (LangGraph, MCP, your own loop).")
        print("If your tools are plain functions, the simpler path is:")
        print()
        print("  tool-pouch init && tool-pouch scan --quick")
        print()
        print("To stay on `tool-pouch run`, either:")
        print("  1. tool-pouch run path/to/agent.py")
...
def _latest_run_id_or_exit():
    ...
    if not rid:
        print("No runs found. Run `tool-pouch run` first.")
        sys.exit(2)
    return rid
...
def cmd_scan(args):
    """Auto-discover @tool_pouch.tool functions and stress-test them.
    ...
    if not tools_path:
        print("Error: no tools path. Pass `tool-pouch scan <path>` or set "
    ...
    if not tools:
        print(f"No @tool_pouch.tool functions found under {tools_path}.")
    ...
    if args.quick and not scenarios:
        ...
        print(f"Quick mode: {len(scenarios)} scenario(s), 1 input "
              f"(skip with full `tool-pouch scan`)", file=sys.stderr)
...
def cmd_init(args):
    ...
    if plan.tools_path:
        ...
        print(f"  tools:    (none detected — set this once you have @tool_pouch.tool functions)")
    print()
    if plan.tools_path:
        print("Next: tool-pouch scan --quick")
    else:
        print("Next: decorate a function with @tool_pouch.tool, set `tools` in .tool_pouch.toml,")
        print("      then run `tool-pouch scan --quick`.")
...
def cmd_traces(args):
    ...
    if not rows:
        ...
        print()
        print("  client = tool_pouch.wrap_openai(OpenAI())")
        print("  # ...")
        sys.exit(0)
    ...
    print()
    print(col.dim(f"Drill into a trace:  tool-pouch trace <trace_id>"))
    print(col.dim(f"Replay a trace:      tool-pouch replay <trace_id>"))
...
def cmd_sync(args):
    ...
    print()
    print("When the cloud ships, `tool-pouch sync` will push traces to your")
    ...
    print()
    print("  client = tool_pouch.wrap_openai(")
    print("      client,")
    print("      destinations=[tool_pouch.JSONLogger()],")
    print("  )")
    print()
    ...
    print("  client = tool_pouch.wrap_openai(")
    print("      client,")
    print('      destinations=[tool_pouch.HTTPSink(url="https://your.api/traces")],')
    print("  )")
    print()
...
def _maybe_nudge_cloud(trace_count: int) -> None:
    if trace_count >= 5_000:
        nudges_mod.show_once(
            "trace_count_5k",
            "\n[!] You've captured 5k+ local traces. `tool-pouch sync` will push "
...
{first_time_label}
  tool-pouch init                  Autodetect tools/provider/model, write .tool_pouch.toml
  tool-pouch scan --quick          Fastest pre-deploy stress test (~15s)

{test_label}
  init                            Autodetect setup and write .tool_pouch.toml
  scan [path]                     Auto-discover @tool_pouch.tool functions and test
...
{flags_label}
  ...
  --provider openai | anthropic   (scan) Provider for the agent under test
  --model <name>                  (scan) Model name (e.g. gpt-4o, claude-opus-4-7)
...
{examples_label}
  tool-pouch init && tool-pouch scan --quick              {ex_scan_comment}
  tool-pouch scan ./tools/ --provider anthropic --model claude-opus-4-7
  tool-pouch run my_agent.py --scenarios timeout,malformed_json
  tool-pouch traces --since 1h --failed              {ex_traces_comment}
  tool-pouch trace --request-id req-abc123           {ex_trace_comment}
  tool-pouch replay <trace_id> --repeat 100          {ex_replay_comment}
  tool-pouch replay <trace_id> --frozen              {ex_frozen_comment}
  tool-pouch fix-prompt | pbcopy                     {ex1_comment}

{tips_label}
  • `tool-pouch scan` is the recommended entrypoint for new projects.
  • `tool_pouch.wrap_openai(client)` captures every production request to the
    local store. Drop in `JSONLogger()` or `HTTPSink(url=...)` for prod.
  • `tool-pouch replay` re-runs a captured trace under chaos by default, so
    you can answer "would this incident reproduce?" in one command.
  • Drop a `.tool_pouch.toml` in your project root and `tool-pouch scan` / `tool-pouch run`
...
def main():
    ...
    sc = sub.add_parser("scan",
                        help="Auto-discover @tool_pouch.tool functions and stress-test")
